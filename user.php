...
<body>
    ...
    if (isset($_POST['submit'])) {
        ...
        $pswd = $_POST['pswd'];
        $sql = "Select * from login_master where loginid = $loginid and pswd='$pswd'";
        $query = mysqli_query($con, $sql);
        if (!$query) {
            ...
            if (empty($data)) {
                ...
                $sql = "update login_master set status = 'logged in' where loginid = $loginid";
                $query = mysqli_query($con, $sql);
                echo "<script>alert('You have successfully logged in');</script>";
                echo "<script type='text/javascript'> document.location ='user_options.php'; </script>";
            }
        }
    }
    ?>
